...
// ****************************************************************************
	...
	addEvent("click", document.getElementById("data_uri_clicker"), function () {
		...
		if (datauri != null && datauri.length > 0) {
			try {
				console.log(Math.floor(datauri.length * 3 / 4) + " bytes of data submitted by form (text length of " + datauri.length + ").", 0);
				initPlayer();
	...
	addEvent("change", document.getElementById("local_file_open"), function () {
		...
		if (typeof this.files != "undefined") {
			console.log("files: ", typeof(this.files));
			console.log("files len: ", this.files.length);
			try {
				if (this.files.length >= 1) {
					console.log("Reading the local file \"" + this.files[0].name + "\"", 0);
					try {
						...
						binaryHandle.onload = function () {
							if (this.readyState == 2) {
								...
								console.log("loading file, please wait...", 0);
							}
						}
						var readThis = this.files[this.files.length - 1];
						console.log(readThis);
						binaryHandle.readAsBinaryString(readThis);
					}
					catch (error) {
						console.log("Browser does not support the FileReader object, falling back to the non-standard File object access,", 2);
					...
					console.log("Incorrect number of files selected for local loading.", 1);
				}
			}
			catch (error) {
				console.log("Could not load in a locally stored ROM file.", 2);
			}
		}
		else {
			console.log("could not find the handle on the file to open.", 2);
		}
	});
	addEvent("change", document.getElementById("save_open"), function () {
		...
		if (typeof this.files != "undefined") {
			try {
				if (this.files.length >= 1) {
					console.log("Reading the local file \"" + this.files[0].name + "\" for importing.", 0);
					try {
						//Gecko 1.9.2+ (Standard Method)
						var binaryHandle = new FileReader();
						binaryHandle.onload = function () {
							if (this.readyState == 2) {
								console.log("file imported.", 0);
								try {
								...
								console.log("importing file, please wait...", 0);
							}
						}
